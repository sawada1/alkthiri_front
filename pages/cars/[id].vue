<template>
  <div class="min-h-screen mb-16">
    <div class="container">
      <div class="rounded-b-[12px] w-full pt-8 bg-white shadow-shadow1">
        <h1
        ...
        <div class="flex items-end justify-between xl:flex-row lg:flex-row flex-col">
            <CarPrice></CarPrice>
            <div class="w-full xl:w-fit lg:w-fit px-4 xl:px-0 lg:px-0 mt-4 xl:mt-0 lg:mt-0">
       <CarColors></CarColors>
            </div>
          <div class="px-4 xl:px-0 lg:px-0 xl:w-fit lg:w-fit w-full mt-5 xl:mt-0 lg:mt-0 mb-2 xl:mx-2 lg:mx-2 mx-0">
            <nuxt-link  v-if="store.CarDetails.price_field_status != 'unavailable'" :to="$localePath({path:'/purchase' , query:{id: store.CarId , model_id:route.params.id , color_id: store.ColorId}})">
              <UButton
      ...
      <div class="flex flex-col gap-10">
        ...
        <swiper-container  class="priceSwiper" ref="containerRef3">
          <swiper-slide
            ...
            @click="checkCar = index , store.CarDetails = item , store.CarId = item.id"
            class="flex items-center text-[#767676] p-2 cursor-pointer text-nowrap rounded-t-lg justify-center"
            :class="{'bg-[#dadfe6] text-primary font-bold': checkCar == index}"
          >
           <span> {{ item.name }} </span>
          </swiper-slide>
        </swiper-container>
      </ClientOnly>
      <button v-if="store.cars.length >= 3" @click="swiper2.next()" class="bg-primary absolute top-[15%] rtl:left-[-15px] ltr:right-[-15px]  rounded-full z-20">
        <svg
       ...
       <div class="w-full h-[1px] bg-[#767676]"></div>
         ...
         <p class="text-tw-grey text-[18px]"> {{ store.CarDetails?.description }} </p>
      </div>
      <div>
